...
// Number of tampons that should fit into the dispenser
Capacity = 30;
...
tampon_opening_height = tampon_diameter * 3.5;
...
module tampon_cutouts() {
	translate([Wall_Thickness, Wall_Thickness/2 - size.y/2, tampon_diameter])
	rotate([90, 0, 0])
	...
	translate([
		...
		tampon_diameter + tampon_opening_height + (size.z - tampon_diameter - tampon_opening_height) / 2
	])
	rotate([90, 0, 0])
	linear_extrude(Wall_Thickness/2)
...
module tampon_insides() {
	// top slope
	top_slope_width = min(size.y - Diameter - 2*Tolerance_Tampons_Diameter, tampon_opening_height);
	multmatrix(shear(-Shear_Factor*2))
	translate([Wall_Thickness, -size.y/2, -Wall_Thickness/2 + tampon_diameter*3.25])
